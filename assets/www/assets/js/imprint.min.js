...
function onGeoSuccess(position){
    ...
    if(current == 'home.html' || current == 'peta-home.html')
        goCariDokter();
}
...
function onNotification(e) {
	switch( e.event )
	{
		...
		case 'message':
			if (e.foreground)
			{
				var my_media = new Media("/android_asset/www/assets/sound/notif.mp3");
				my_media.play();
				var loc = window.location.pathname.split('/');
					loc = loc[4];
					
				if(loc == 'home.html' || loc == 'antrian.html' || loc == 'notif.html' || loc == 'lain.html'){
					$(".notif").show();
					//var unread_notif = 1;
					if (localStorage) {
						if(localStorage.unread){
							unread_notif = parseInt(localStorage.unread)+1 ;
						}
						else
							unread_notif = 1;
					}else
					...
					localStorage.unread = unread_notif;
					
					$(".notif").text((unread_notif));
				}
			}
			else
			{	if (e.coldstart)
